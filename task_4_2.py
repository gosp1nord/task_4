...
def insert_tables():
    ...
    singers = [
        ('дед Мазай', ('кантри', 'романс'), (('Я - лучший', 2008), ('Быстрый заяц', 2010))),
        ('Лосяш', ('рок', 'хип-хоп', 'регги'), (('Прекрасные рога', 2009), ('Споем, друг Копатыч', 2010), ('Тайна Кузинатры', 2011))),
    ...
    tracks = [
        ('На вершине горы', '236', 'Я - лучший', (('The BEST', 2015), ('Северный ветер', 2019))),
        ('Бабочка', '288', 'Прекрасные рога', (('Солнечная система', 2011), ('Есть ли жизнь на Марсе', 2014), ('Легенды', 2020))),
        ...
        ('Песня без слов', '307', 'Звезда по имени Солнце', (('Песня без слов', 1989), ('Последний герой', 1988))),
    ...
    for item in tracks:
        ...
        if not id_album:
            print(f'В списке альбомов название альбома "{item[2]}", в который включен трек {item[0]} не найдено.')
            continue
